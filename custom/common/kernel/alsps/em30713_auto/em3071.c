...
//#define CUST_EINT_ALS_POLARITY         CUST_EINT_POLARITY_LOW //modify by linhui
...
#define PS_DRIVE   0xB8//0XA1  //50MA  //0XB8 //200MA  0XB6 100MA  //Psensor 驱动力
/*----------------------------------------------------------------------------*/
...
static int em3071_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);
/*----------------------------------------------------------------------------*/
...
static int  em30713_local_init(void);
static int  em30713_remove(void);
static int em30713_init_flag =-1; // 0<==>OK -1 <==> fail
static struct sensor_init_info em30713_init_info = {
		.name = "em30713",
		.init = em30713_local_init,
		.uninit = em30713_remove,
};
...
//    u16		    als_modulus;
    ...
    atomic_t    ps_thd_val_high;     /*the cmd value can't be read, stored in ram*/  //modify by linhui
	atomic_t    ps_thd_val_low;     /*the cmd value can't be read, stored in ram*/   //modify by linhui
	atomic_t    ps_thd_val;
...
int em3071_setup_eint(struct i2c_client *client)
{
	...
	//mt_eint_set_sens(CUST_EINT_ALS_NUM, CUST_EINT_ALS_SENSITIVE); //modify by linhui
	//	mt_eint_set_polarity(CUST_EINT_ALS_NUM, CUST_EINT_ALS_POLARITY); //modify by linhui
	#ifdef CUST_EINT_ALS_TYPE
		mt_eint_set_hw_debounce(CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_CN);
		mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_TYPE, em3071_eint_func, 0);
	#else
...
/*singal interrupt function add*/
	...
	//printk("zijian em3071_eint_work ,mt_eint_unmask\n ");
	#ifdef CUST_EINT_ALS_TYPE
		mt_eint_unmask(CUST_EINT_ALS_NUM);
	#else
		mt65xx_eint_unmask(CUST_EINT_ALS_NUM);
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				APS_ERR("em3071_ps_operate ps data=%d!\n",obj->ps); //
...
static int em3071_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = em30713_get_cust_alsps_hw(); 
	...
	atomic_set(&obj->ps_thd_val_high,  obj->hw->ps_threshold_high); //modify by linhui
	atomic_set(&obj->ps_thd_val_low,  obj->hw->ps_threshold_low);//modify by linhui
...
{
	struct alsps_hw *hw = em30713_get_cust_alsps_hw(); 
...
static int  em30713_remove(void)
{
    struct acc_hw *hw = em30713_get_cust_alsps_hw(); 
...
//	i2c_em3071.addr = hw->i2c_addr[0] ;
	i2c_register_board_info(hw->i2c_num, &i2c_em3071, 1);
		...
		//hwmsen_psensor_add(&em30713_init_info);
		hwmsen_alsps_sensor_add(&em30713_init_info);
#if 0
