...
#include "tmd2771.h"
#include <linux/wakelock.h>//modify by linhui
...
int TMD2771_CMM_PPCOUNT_VALUE = TMD2771_CMM_PPCOUNT;
int TMD2771_CMM_CONTROL_VALUE = TMD2771_CMM_CONFIG;
int ZOOM_TIME = 10;
...
static long TMD2771_enable_ps(struct i2c_client *client, int enable)
{
	...
	if(enable)
		...
		wake_unlock(&ps_lock);//modify by linhui
...
//	mt_eint_set_polarity(CUST_EINT_ALS_NUM, CUST_EINT_ALS_POLARITY);
		...
		mt65xx_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, TMD2771_eint_func, 0);
	#endif
...
static int TMD2771_init_client(struct i2c_client *client)
{
	...
	databuf[1] = 0x08;//TMD2771_CMM_PPCOUNT_VALUE;
	res = TMD2771_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
	if(res <= 0)
	{
		goto EXIT_ERR;
	...
	databuf[1] = 0x20;//TMD2771_CMM_CONTROL_VALUE;
	res = TMD2771_i2c_master_operate(client, databuf, 0x2, I2C_FLAG_WRITE);
	if(res <= 0)
	{
		goto EXIT_ERR;
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				sensor_data->values[0] = TMD2771_get_ps_value(obj, obj->ps);
				printk("TMD2771_ps_operate ps data=%x!\n",obj->ps);
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;			
			}
			break;
...
static int TMD2771_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	////////
	wake_lock_init(&ps_lock, WAKE_LOCK_SUSPEND, "ps wakelock"); //modify 
	/*for interrup work mode support -- by liaoxl.lenovo 12.08.2011*/
...
static int  TMD2771_remove(void)
{
    struct acc_hw *hw = TMD2771_get_cust_alsps_hw();
