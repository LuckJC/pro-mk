...
#define CH7035_I2C_SLAVE_ADDR       0x52
/*----------------------------------------------------------------------------*/
...
static int ch7035_exit(void) //ch7035 power off
{
    ...
    mt_set_gpio_dir(GPIO92, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO92, GPIO_OUT_ZERO);  // Reset EP956E
...
static int ch7035_video_config(HDMI_VIDEO_RESOLUTION vformat, HDMI_VIDEO_INPUT_FORMAT vin, HDMI_VIDEO_OUTPUT_FORMAT vout)
{
    ...
    {
        case HDMI_VIDEO_720x480p_60Hz:
            inputVIC = HDMI_RX_OUTPUT_480p;	
            break;
        case HDMI_VIDEO_1280x720p_60Hz:
            inputVIC = HDMI_RX_OUTPUT_720p;	
            break; 
	 case HDMI_VIDEO_1920x1080p_30Hz:
            inputVIC = HDMI_RX_OUTPUT_1080p30;
            break; 
        default:
            inputVIC = HDMI_RX_OUTPUT_720p;
            break;
    ...
    switch(vout)
    {
        case HDMI_VOUT_FORMAT_RGB888:
            rxOutput |= HDMI_RX_OUTPUT_RGB << 4;
            break;
        case HDMI_VOUT_FORMAT_YUV422:
            rxOutput |= HDMI_RX_OUTPUT_YUV422 << 4;
            break;    
	 case HDMI_VOUT_FORMAT_YUV444:
            rxOutput |= HDMI_RX_OUTPUT_YUV444 << 4;
            break;    
        default:
            rxOutput |= HDMI_RX_OUTPUT_RGB << 4;
            break;
    ...
    EP955_UserLevel_VideoConfig(rxOutput, inputVIC);
...
static int ch7035_audio_config(HDMI_AUDIO_FORMAT aformat)
{
    CH7035_FUNC();
    //RETINT(0 == ucHdmi_chip_exist, 0);
    EP955_UserLevel_AudioConfig(true, AUD_FREQ_44100Hz);
    return 0;
}
/*----------------------------------------------------------------------------*/
...
{        
    ...
    mt_set_gpio_dir(GPIO92, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO92, GPIO_OUT_ZERO);  // Reset EP956E
    //if(hdmi_util.mdelay == NULL)  return -1;
    msleep(100);
    //hdmi_util->mdelay(100);  // Hold the Reset Signal at least 1ms
    mt_set_gpio_out(GPIO92, GPIO_OUT_ONE); // Reset EP956E
    //hdmi_util->mdelay(500); // Delay 400ms to wait Rx ready
    msleep(500);
...
/*----------------------------------------------------------------------------*/
void ep955_int_cbk(CBUS_LINK_STATE status)
{
    ;
}
...
{             
    ...
    EP955_UserLevel_InitMemory(ch7035_i2c_client);
    EP955_UserLevel_Init(ep955_int_cbk);
...
#endif	
    int ret;
    ret = EP955_UserLevel_GetState();
    switch(ret) {
        case CBUS_LINK_STATE__USB_Mode:
            ret = HDMI_STATE_NO_DEVICE;
            break;
	 case CBUS_LINK_STATE__1KOHM_Detected:
            ret = HDMI_STATE_CONNECTING;
            break;
	 case CBUS_LINK_STATE__Start_Connect:
            ret = HDMI_STATE_CONNECTING;
            break;
	 case CBUS_LINK_STATE__Check_DCAP:
            ret = HDMI_STATE_ACTIVE;
            break; 
	 case CBUS_LINK_STATE__Connected:
            ret = HDMI_STATE_ACTIVE;
            break;
        default:
            ret = HDMI_STATE_NO_DEVICE;
