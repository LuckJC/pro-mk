...
#define OV8865_DEBUG
//#ifdef OV8865_DEBUG
	//#define printk(fmt, arg...) xlog_printk(ANDROID_LOG_DEBUG, "[OV8865Raw] ",  fmt, ##arg)
//#else
//	#define printk(fmt, arg...)
//#endif

//OV FAE modify for mirror start
#if 0
static const ACDK_SENSOR_IMAGE_MIRROR_ENUM rotate_map[4] = {
	ACDK_SENSOR_IMAGE_V_MIRROR,
	ACDK_SENSOR_IMAGE_HV_MIRROR,
	ACDK_SENSOR_IMAGE_NORMAL,
	ACDK_SENSOR_IMAGE_H_MIRROR,
};
#endif
//OV FAE modify for mirror end
...
kal_uint32 OV8865_CalcFrameLength_For_AutoFlicker(void)
{
	...
	printk("AutoFlicker_min_framelength =%d,OV8865CurrentScenarioId =%d\n", AutoFlicker_min_framelength,OV8865CurrentScenarioId);
...
void OV8865_write_shutter(kal_uint32 shutter)
{
	...
	printk("ov8825 write shutter=%x, line_length=%x, frame_length=%x\n", shutter, line_length, frame_length);
...
void write_OV8865_gain(kal_uint16 gain)
{
	...
	//printk("[OV8865_SetGain]0x3508|0x3509=0x%x \n",read_gain);
...
void OV8865_SetGain(UINT16 iGain)
{
	...
	printk(" [OV8865_SetGain]ov8865.sensorGlobalGain=0x%x,ov8865.realGain =%x",ov8865.sensorGlobalGain,
		ov8865.realGain); 

	//temperature test
	//OV8865_write_cmos_sensor(0x4d12,0x01);
	//printk("Temperature read_reg  0x4d13  =%x \n",OV8865_read_cmos_sensor(0x4d13));
}   
...
kal_uint16 read_OV8865_gain(void)
{
	...
	printk("ov8865.sensorGlobalGain=0x%x,ov8865.realGain=%d\n",ov8865.sensorGlobalGain,ov8865.realGain);
...
{
	printk(" OV8865PreviewSetting_2lane enter\n");
	...
	OV8865_write_cmos_sensor(0x3821,0x61);//; hsync_en_o, fst_vbin, mirror on //0x67
...
{
	printk(" OV8865videoSetting_2lane enter:video/preview sync\n");
...
{
	printk("OV8865CaptureSetting_2lane enter\n");
	...
	OV8865_write_cmos_sensor(0x3821,0x40);// ; hsync_en_o, fst_vbin, mirror on 0x46
...
{
	printk("OV8865_Sensor_Init_2lane enter\n");
	...
	OV8865_write_cmos_sensor(0x3820,0x06);// ; flip off //0x00
	OV8865_write_cmos_sensor(0x3821,0x61);// ; hsync_en_o, fst_vbin, mirror on //0x67
	OV8865_write_cmos_sensor(0x382a,0x03);// ; Y inc odd
	OV8865_write_cmos_sensor(0x382b,0x01);// ; Y inc even
	OV8865_write_cmos_sensor(0x3830,0x08);// ; ablc_use_num[5:1]
...
UINT32 OV8865Open(void)
{
	...
	for(i=0;i<2;i++)
	{
		...
		printk("OV8865 READ ID :%x",sensor_id);
    ...
    do {
        *sensorID = (OV8865_read_cmos_sensor(0x300B)<<8)|OV8865_read_cmos_sensor(0x300C);
        if (*sensorID == OV8865_SENSOR_ID)
        	{
        		printk("Sensor 8865 ID = 0x%04x\n", *sensorID);
            	break;
        	}
        printk("Read Sensor 8865 ID Fail = 0x%04x\n", *sensorID);
        retry--;
    } while (retry > 0);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	printk("OV8865Preview enter:");

	OV8865PreviewSetting();
	
	//ov FAE modify for mirror start
	#if 0
	OV8865SetFlipMirror(rotate_map[sensor_config_data->SensorImageMirror]);
	#endif
	//ov FAE modify for mirror end
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	{
		printk("OV8865Capture BusrtShot / ZSD!!!\n");
	}
	else
	{
		printk("OV8865Capture enter:\n");
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
		...
		printk("OV8865CurrentScenarioId=%d\n",OV8865CurrentScenarioId);
	...
	switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            ...
            break;
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			printk("OV8865 video_preiew sync\n");
...
UINT32 OV8865SetVideoMode(UINT16 u2FrameRate)
{
    ...
    printk("[OV8865SetVideoMode] frame rate = %d\n", u2FrameRate);
	...
	if(u2FrameRate >30 || u2FrameRate <5)
	    printk("abmornal frame rate seting,pay attention~\n");
	...
	printk("[OV8865SetVideoMode]MIN_Frame_length=%d,ov8865.DummyLines=%d\n",MIN_Frame_length,ov8865.DummyLines);
...
UINT32 OV8865SetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
	...
	if(bEnable) {   
		...
		spin_unlock(&ov8865mipiraw_drv_lock);
        printk("OV8865 Enable Auto flicker\n");
    } else {
    	spin_lock(&ov8865mipiraw_drv_lock);
        ov8865.OV8865AutoFlickerMode = KAL_FALSE;
		spin_unlock(&ov8865mipiraw_drv_lock);
        printk("OV8865 Disable Auto flicker\n");
    }
...
{
    printk("[OV8865SetTestPatternMode] Test pattern enable:%d\n", bEnable);
    if(bEnable == KAL_TRUE)
    {
...
UINT32 OV8865MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	...
	printk("OV8865MIPISetMaxFramerateByScenario: scenarioId = %d, frame rate = %d\n",scenarioId,frameRate);
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
