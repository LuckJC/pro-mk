...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("FM50AF", 0x1C)};


#define FM50AF_DRVNAME "FM50AF"
#define FM50AF_VCM_WRITE_ID           0x1C
...
static int s4FM50AF_WriteReg(u16 a_u2Data)
{
	...
	i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, VCMMSB, 2);
	i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, VCMLSB, 2);
...
static int FM50AF_Mode_Init(int a)
{
	int  i4RetValue = 0;
	char Move[2] = {(char)(0x02) , (char)(0x19)};
	char MoveTime[2] = {(char)(0x06) , (char)(0x02)};

	g_pstFM50AF_I2Cclient->addr = 0x0E;

	i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, Move, 2);
	i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, MoveTime, 2);
	mode_init = 0;
	return 0;
}
