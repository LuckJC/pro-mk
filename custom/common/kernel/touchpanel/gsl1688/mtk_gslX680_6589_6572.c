...
#define TPD_KEYS_DIM	{{108, 1320, 60, 50},{396, 1320, 60, 50},{612, 1320, 60, 50},{470, 2048, 60, 50}}
#endif
...
static void judge_chip_type(struct i2c_client *client)
{
	...
	ret = i2c_smbus_read_i2c_block_data(client,0xfc, read_buf, sizeof(read_buf));
		if  (ret  < 0)  
    		rc --;
	else
		printk("I read reg 0xfc is %x\n", read_buf);
...
static void gsl_load_fw(struct i2c_client *client)
{
	...
	printk("------gslX680 gsl_load_fw xianliang------\n");	
...
static int test_i2c(struct i2c_client *client)
{
	...
	printk("------gslX680 test_i2c xianliang------\n");	
...
static void init_chip(struct i2c_client *client)
{
	int rc;
		...
		printk("------gslX680 init_chip xianliang------\n");	
...
static void check_mem_data(struct i2c_client *client)
{
	...
	printk("------gslX680 check_mem_data xianliang------\n");	
...
static int gsl_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	printk("------gslX680 gsl_config_read_proc xianliang------\n");	
...
static int gsl_config_write_proc(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	printk("------gslX680 gsl_config_write_proc xianliang------\n");	
	if(count > 512)
	{
...
{
	print_info("------tpd_up------ \n");
	printk("------gslX680 tpd_up xianliang------\n");	
...
static void report_data_handle(void)
{
	...
	if(i2c_lock_flag != 0)
		...
		i2c_lock_flag = 1;
#endif
printk("------gslX680 report_data_handle xianliang------\n");	
...
static int touch_event_handler(void *unused)
{
	...
	printk("------gslX680 touch_event_handler xianliang------\n");	
	do
	{
...
void tpd_eint_interrupt_handler(void)
{
	...
	printk("------gslX680 tpd_eint_interrupt_handler xianliang------\n");	
...
static int tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	...
	msleep(100);
        ...
        hwPowerOn(MT6323_POWER_LDO_VGP1, VOL_2800, "TP");//2800
	msleep(100);
