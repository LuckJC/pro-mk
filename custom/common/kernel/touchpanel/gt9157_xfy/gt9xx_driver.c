...
static int gt91xx_config_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    ptr += sprintf(ptr, "Driver VID: 0x%02X%02X\n", gtp_default_FW[12], gtp_default_FW[13]);  //xianliang modify
...
static int touch_event_handler(void *unused)
{
	...
	else
	{
        ...
        pre_touch = touch_num;
        input_report_key(tpd->dev, BTN_TOUCH, (touch_num || key_value)); //xianliang modify
