...
#include "gt9xx_ex_fun.h"
#endif
#define TPD_OK 0
#define I2C_DEV_NUMBER		8
struct i2c_client *i2c_client = NULL;

#define TPD_INFO(fmt, arg...)  printk("[tpd info:gt9xx]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
#define TPD_DEBUG(fmt, arg...)  printk("[tpd debug:gt9xx]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
#define TPD_DMESG(fmt, arg...)  printk("[tpd dmesg:gt9xx]" "[%s]" fmt "\r\n", __FUNCTION__ ,##arg)
...
static s8 gtp_enter_sleep(struct i2c_client *client);//wwj
//-----------------------------------------------------
//wwj-add
...
static ssize_t wakeup_store(struct kobject *kobj,struct kobj_attribute *attr,const char * buf, size_t n);
...
static ssize_t wakeup_store(struct kobject *kobj,struct kobj_attribute *attr,const char * buf, size_t n)
{
	...
	if (EnableWakeUp != symbol) {
		...
		/* Modify wake up mode:
			...
			2: only 5 fingers
			3: 2 or 5 fingers*/
		//ret = gt9xx_write_reg(i2c_client, 0xac, EnableWakeUp);
...
#else
#ifdef CUST_EINT_TOUCH_PANEL_TYPE
...
#ifdef SYSFS_DEBUG
	gt9xx_create_sysfs(i2c_client);
#endif

#ifdef FTS_APK_DEBUG
	gt9xx_create_apk_debug_channel(i2c_client);
#endif
...
//wwj-add
	...
	#ifdef FTS_APK_DEBUG
	gt9xx_release_apk_debug_channel();
	#endif
	#ifdef SYSFS_DEBUG
	gt9xx_release_sysfs(client);
	#endif
...
static int touch_event_handler(void *unused)
{
	...
	//wwj add end
	    ...
	    GTP_INFO("--------------wwj---%c--------------",doze_buf[2]);
            if (ret > 0)
            {    
            	/*           
                if ((doze_buf[2] == 'a') || (doze_buf[2] == 'b') ||
                    (doze_buf[2] == 'd') || (doze_buf[2] == 'g') || 
                    (doze_buf[2] == 'h') || (doze_buf[2] == 'q') || 
		            (doze_buf[2] == 'v') || 
                    (doze_buf[2] == 'y') || (doze_buf[2] == 'z') ||
                    (doze_buf[2] == 0x5E) //^
                    )*/
                    //if ((doze_buf[2] == 'w')||(0xCC == doze_buf[2]))  
		if (doze_buf[2] == 'w')
                {
                	/*
                    if (doze_buf[2] != 0x5E)
		...
		//wwj add
		else if (doze_buf[2] == 's') //MESSAGING
		{
		    	doze_status = DOZE_WAKEUP;
		    ...
		    doze_status = DOZE_WAKEUP;
                ...
                */
                else
                {
                    ...
                    doze_buf[2] = 0x00;
                    gtp_i2c_write(i2c_client_point, doze_buf, 3);
                    //wwj gtp_enter_doze(i2c_client_point);
                    //wwj

                    if(EnableWakeUp==1)//if(gestureSwitch==1)
										{
