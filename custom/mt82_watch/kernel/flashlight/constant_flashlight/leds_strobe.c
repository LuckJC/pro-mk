...
Functions
*****************************************************************************/
#define GPIO_ENF GPIO12 //GPIO_CAMERA_FLASH_EN_PIN
#define GPIO_ENT GPIO13 //GPIO_CAMERA_FLASH_MODE_PIN
...
int FL_Enable(void)
{
	if(g_duty==0)
	{
		...
		*/
		mt_set_gpio_out(GPIO_ENT,GPIO_OUT_ZERO); //ONE-->ZERO
		mt_set_gpio_out(GPIO_ENF,GPIO_OUT_ONE); //ZERO-->ONE
	...
	else
	{
		...
		*/
		mt_set_gpio_out(GPIO_ENT,GPIO_OUT_ONE);//ZERO-->ONE
		mt_set_gpio_out(GPIO_ENF,GPIO_OUT_ONE);
		PK_DBG(" FL_Enable line=%d\n",__LINE__);
	}
