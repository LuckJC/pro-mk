...
unmount_mounted_volume(const MountedVolume *volume)
{
    ...
    if (ret == 0) {
        printf("umount %s sucess!! no process has been killed \n",volume->mount_point);
    }
    else
    {
    	printf("umount %s fail, ret=%d, errno:%d ,try call superumount \n",volume->mount_point,ret, errno);
		status=system("/system/bin/superumount -p /data");
		if(status < 0){
			printf(" Excute /system/bin/superumount fail ! ,error message: %d\n",errno);
			ret=umount2(volume->mount_point,MNT_DETACH);	
		}else if(WIFSIGNALED(status)){
			printf(" Excute /system/bin/superumount fail ! stop by signal,singal number: %d\n", WTERMSIG(status));
			ret=umount2(volume->mount_point,MNT_DETACH);
		}else if(WIFEXITED(status)){
			printf(" Excute /system/bin/superumount OK\n");
			ret = 0;
		}
    }
