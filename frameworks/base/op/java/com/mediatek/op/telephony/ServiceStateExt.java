...
public class ServiceStateExt implements IServiceStateExt {
    ...
    public int mapGsmSignalLevel(int asu,int GsmRscpQdbm){
        ...
        if (GsmRscpQdbm < 0) {
        		int rssi = GsmRscpQdbm/4;
            // 3G network
            if (asu <= 2 || asu == 99) level = SignalStrength.SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
            else if (rssi >= -90 /*asu >= 15*/) level = SignalStrength.SIGNAL_STRENGTH_GREAT;
            else if (rssi >= -99 /*asu >= 12*/)  level = SignalStrength.SIGNAL_STRENGTH_GOOD;
            else if (rssi >= -105 /*asu >= 9*/)  level = SignalStrength.SIGNAL_STRENGTH_MODERATE;
