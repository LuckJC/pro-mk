                           ...
                           try {
                               ...
                               while (!cursorName.isAfterLast()) {
                                   ...
                                   if (name.equals(tempName)) {
                                       ...
                                       Log.v(TAG, "updateUICallback1..name not need update:");
                                       break;
                                   }
                                   newName = tempName;//update the same
                                   isNeedUpdateName = true;
            ...
            if (!cursor.isAfterLast()) {
                ...
                try {
                    if (!viewcursor.isAfterLast()) {
                        ...
                        dataid = allWhite(viewcursor.getString(CALLSVIEW_DATA_ID_COLUMN)); 
                        Log.v(TAG, "updateCallLogNumbers1----[number:" + number + "]----[data_id" + dataid + "]");
                    }
                } finally {
                    viewcursor.close();
                }
                ...
                if (dataid == null || dataid.isEmpty()) {
                    ...
                    try {
                        if (!cursorDataid.isAfterLast()) {
                            rawid = allWhite(cursorDataid.getString(DATA_RAW_CONTACT_ID_COLUMN));
