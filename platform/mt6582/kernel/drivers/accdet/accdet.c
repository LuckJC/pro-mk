...
static void accdet_eint_func(void)
{
	...
	*/
		if (CUST_EINT_ACCDET_TYPE == CUST_EINTF_TRIGGER_HIGH){
					mt_eint_set_polarity(CUST_EINT_ACCDET_NUM, (0));//1 xianliang modify
		}else{
					mt_eint_set_polarity(CUST_EINT_ACCDET_NUM, (1));//0 xianliang modify
		}
...
static inline int accdet_setup_eint(void)
{
    ...
    mt_set_gpio_pull_enable(GPIO_ACCDET_EINT_PIN, GPIO_PULL_ENABLE); //To disable GPIO PULL.
...
static inline int accdet_setup_eintEx(void)
{
...
int mt_accdet_probe(void)	
{
	...
	{	
		...
		mt_set_gpio_mode(GPIO20, 0);
		mt_set_gpio_dir(GPIO20, GPIO_DIR_OUT);
		mt_set_gpio_pull_enable(GPIO20, GPIO_PULL_ENABLE); //To disable GPIO PULL.
